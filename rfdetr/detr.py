...
class RFDETR:
    ...
    def __init__(self, **kwargs):
        print("正在初始化 RF-DETR 模型...")
        self.model_config = self.get_model_config(**kwargs)
        print("正在下载/加载预训练权重（如果尚未下载，这可能需要几分钟）...")
            ...
            try:
                ...
                if ann_file and os.path.exists(ann_file):
                    with open(ann_file, 'r', encoding='utf-8') as f:
                        ...
                        self.model.class_names = class_names
                        logger.info(f"从标注文件读取到 {num_classes} 个类别: {class_names}")
                else:
                    raise FileNotFoundError(f"未找到COCO标注文件")
            except Exception as e:
                logger.warning(f"无法从标注文件读取类别信息: {e}，使用默认COCO类别（90类）")
