...
class Model:
    def __init__(self, **kwargs):
        ...
        # Ensure RF-DETR specific backbone arguments are present when calling build_model.
        # These are normally supplied by higher-level wrappers (e.g., RFDETRBase in rfdetr.detr),
        # but may be missing when invoking the legacy LW-DETR CLI directly.
        base_cfg = RFDETRBaseConfig()

        # Backbone architecture: if encoder is not a windowed DINOv2 variant, fall back to RF-DETR Base defaults.
        if not getattr(args, "encoder", "").startswith("dinov2_windowed"):
            args.encoder = base_cfg.encoder
            args.hidden_dim = base_cfg.hidden_dim
            args.dec_layers = base_cfg.dec_layers
            args.sa_nheads = base_cfg.sa_nheads
            args.ca_nheads = base_cfg.ca_nheads
            args.dec_n_points = base_cfg.dec_n_points
            args.out_feature_indexes = list(base_cfg.out_feature_indexes)
            args.projector_scale = list(base_cfg.projector_scale)

        # Resolution / positional encodings / windowing: prefer CLI values if provided, otherwise use RF-DETR Base.
        if not hasattr(args, "resolution") or args.resolution is None:
            args.resolution = base_cfg.resolution
        if not hasattr(args, "patch_size") or args.patch_size is None:
            args.patch_size = base_cfg.patch_size
        if not hasattr(args, "num_windows") or args.num_windows is None:
            args.num_windows = base_cfg.num_windows
        if not hasattr(args, "positional_encoding_size") or args.positional_encoding_size is None:
            args.positional_encoding_size = base_cfg.positional_encoding_size
        ...
        if args.pretrain_weights is not None:
            ...
            if args.pretrain_exclude_keys is not None:
                ...
                for exclude_key in args.pretrain_exclude_keys:
                    if any(ch in exclude_key for ch in ["*", "?", "["]):
                        for k in list(checkpoint["model"].keys()):
                            if fnmatch.fnmatch(k, exclude_key):
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        if utils.is_main_process():
            ...
            try:
                ...
                results["checkpoint_mb"] = float(ckpt_path.stat().st_size) / (1024 * 1024)
            except Exception:
                pass
...
def get_args_parser():
    ...
    # Optional external config path (currently not used to overwrite CLI args,
    # but accepted to keep CLI compatible with YAML-driven workflows).
    parser.add_argument('--config', default=None, type=str,
                        help='Optional path to a training/config YAML file.')
    # subparsers
...
def main(**config):
    """
    CLI entry point for LW-DETR / RF-DETR training and evaluation.

    It simply wraps the high-level Model class and delegates to its train()
    method, which internally handles training, evaluation (--eval), and
    optional benchmarking (--do_benchmark) based on the parsed arguments.
    """
    callbacks: DefaultDict[str, List[Callable]] = defaultdict(list)
    model = Model(**config)
    model.train(callbacks=callbacks, **config)
...
def populate_args(
    ...
    # 🚀 CVPR v5.0: SCU 显著性校准不确定性
    use_scu=False,
    scu_salience_weight=0.1,
    scu_calib_slope=-1.5,
    scu_calib_center=-4.2,
    # 🚀 LUE: GIoU precision weighting (Fix 3)
    ...
    **extra_kwargs  # To handle any unexpected arguments
    ...
    args = argparse.Namespace(
        ...
        # 创新点1: AHM参数
        use_adaptive_hybrid=use_adaptive_hybrid,
        ...
        dags_start_epoch=dags_start_epoch,
        dags_warmup_epochs=dags_warmup_epochs,
        dags_bonus_scale=dags_bonus_scale,
