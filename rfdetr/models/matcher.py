...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, focal_alpha: float = 0.25, use_pos_only: bool = False,
        ...
        Params:
            ...
            use_dqcd: If True, degradation-aware matching — reduce cls cost for proposals in degraded regions
        """
        super().__init__()
    ...
    def forward(self, outputs, targets, group_detr=1):
        ...
        if out_log_vars is not None:
            out_log_vars = out_log_vars.flatten(0, 1)  # [batch_size * num_queries, 4]
        ...
        # Compute the GIoU cost between boxes (GIoU more stable for matching than CIoU)
        ...
        # 🚀 v6.0 DAGS: Degradation-aware matching — proposals in degraded regions
        # have less reliable classification scores; reduce cls cost to prevent
        # the matcher from rejecting good localization in degraded areas.
        if self.use_dqcd and 'spatial_prior' in outputs:
            spatial_prior = outputs['spatial_prior']  # (B, 1, H_s, W_s)
            h_sp, w_sp = spatial_prior.shape[2], spatial_prior.shape[3]
            # out_bbox: (B*Q, 4) cxcywh normalised
            px = (out_bbox[:, 0] * (w_sp - 1)).long().clamp(0, w_sp - 1)
            py = (out_bbox[:, 1] * (h_sp - 1)).long().clamp(0, h_sp - 1)
            # Build batch index
            b_idx = torch.arange(bs, device=out_bbox.device).repeat_interleave(num_queries)
            prior_per_query = spatial_prior[b_idx, 0, py, px]  # (B*Q,)
            # Degraded region (low prior) → lower cls cost → easier to match
            deg_weight = (1.0 - 0.3 * (1.0 - prior_per_query)).view(-1, 1)
            cost_class = cost_class * deg_weight
