...
class Transformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, refpoint_embed, query_feat,
                spatial_prior=None, deg_global=None):
        """
        Args:
            spatial_prior: (B, 1, H_s, W_s) optional degradation spatial prior from DegradationEstimator
            deg_global: (B, 4) optional global degradation embedding
        """
        self._spatial_prior = spatial_prior
        ...
        if self.two_stage:
            ...
            for g_idx in range(group_detr):
                ...
                # 🚀 v6.0 DAGS: Degradation-Aware Gated Scoring
                # Classification score is unreliable in degraded regions — add a
                # spatial-prior bonus so that proposals from degraded regions are not
                # crowded out by easy clean-region positives during Top-K selection.
                cls_score = enc_outputs_class_unselected_gidx.max(-1)[0]  # (B, N)
                dags_ratio = getattr(self, '_dags_ratio', 0.0)
                dags_bonus_scale = getattr(self, '_dags_bonus_scale', 0.0)
                if spatial_prior is not None and dags_ratio > 0 and dags_bonus_scale > 0:
                    # Map proposal locations (normalised cx,cy) → spatial_prior grid
                    h_sp, w_sp = spatial_prior.shape[2], spatial_prior.shape[3]
                    # output_proposals: (B, N, 4) [cx, cy, w, h] normalised
                    px = (output_proposals[..., 0] * (w_sp - 1)).long().clamp(0, w_sp - 1)
                    py = (output_proposals[..., 1] * (h_sp - 1)).long().clamp(0, h_sp - 1)
                    batch_idx = torch.arange(px.shape[0], device=px.device).view(-1, 1).expand(-1, px.shape[1])
                    prior_at_proposal = spatial_prior[batch_idx, 0, py, px]  # (B, N) ∈ [0,1]
                    # The estimator's spatial_prior is a detectability map:
                    # low values indicate degraded regions. Keep this bonus
                    # conservative; otherwise Top-K ranking loses AP50.
                    degradability_bonus = (
                        (1.0 - prior_at_proposal) * dags_bonus_scale * dags_ratio
                    )
