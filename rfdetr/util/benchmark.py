...
def benchmark(model, dataset, output_dir):
    ...
    try:
        ...
        _outputs.update({"model_param_mb": float(param_bytes) / (1024 * 1024)})
    except Exception:
        pass
    ...
    with torch.no_grad():
        ...
        for imgid, img in enumerate(tqdm.tqdm(images)):
            inputs = [img.to("cuda")]
            # peak memory (one representative forward; keep it cheap)
            if imgid == warmup_step:
                try:
                    ...
                    _outputs["peak_mem_mb"] = float(peak) / (1024 * 1024)
                except Exception:
                    pass
