...
def complete_box_iou(boxes1, boxes2, eps: float = 1e-7):
    ...
    where:
      ρ² = squared center-point distance
      c²  = squared diagonal of the smallest enclosing box
      v   = aspect-ratio consistency term
      α   = positive trade-off parameter

    Compared to GIoU, CIoU adds:
      1. Center-point distance penalty → gradients even when boxes fully overlap
      2. Aspect-ratio penalty → incentivises matching box shapes, not just areas

    These properties are critical for high-IoU precision (mAP@0.75 → mAP@0.95).
    ...
    Returns:
        ciou: [N, M] pairwise CIoU matrix, values in [-1, 1]
    """
    import math
    ...
    # ---- centre-point distance ρ² and enclosing diagonal c² -----------------
    c1_x = (boxes1[:, 0] + boxes1[:, 2]) * 0.5  # [N]
    c1_y = (boxes1[:, 1] + boxes1[:, 3]) * 0.5
    c2_x = (boxes2[:, 0] + boxes2[:, 2]) * 0.5  # [M]
    c2_y = (boxes2[:, 1] + boxes2[:, 3]) * 0.5

    rho2 = ((c1_x[:, None] - c2_x[None, :]) ** 2 +
            (c1_y[:, None] - c2_y[None, :]) ** 2)  # [N, M]

    # enclosing box: top-left minima, bottom-right maxima
    lt = torch.min(boxes1[:, None, :2], boxes2[:, :2])  # [N, M, 2]
    rb = torch.max(boxes1[:, None, 2:], boxes2[:, 2:])
    wh = (rb - lt).clamp(min=0)
    c2_val = wh[:, :, 0] ** 2 + wh[:, :, 1] ** 2   # diagonal²

    # ---- aspect-ratio penalty v and trade-off α -----------------------------
    w1 = (boxes1[:, 2] - boxes1[:, 0]).clamp(min=eps)  # [N]
    h1 = (boxes1[:, 3] - boxes1[:, 1]).clamp(min=eps)
    w2 = (boxes2[:, 2] - boxes2[:, 0]).clamp(min=eps)  # [M]
    h2 = (boxes2[:, 3] - boxes2[:, 1]).clamp(min=eps)

    atan1 = torch.atan(w1 / h1)  # [N]
    atan2 = torch.atan(w2 / h2)  # [M]
    v = (4.0 / (math.pi ** 2)) * ((atan2[None, :] - atan1[:, None]) ** 2)  # [N, M]

    with torch.no_grad():
        alpha = v / (1.0 - iou + v + eps)

    # ---- CIoU ---------------------------------------------------------------
    ciou = iou - rho2 / (c2_val + eps) - alpha * v
    return ciou


def distance_box_iou(boxes1, boxes2, eps: float = 1e-7):
    """
    Distance IoU (DIoU) — same as CIoU but without the aspect-ratio term.

    DIoU = IoU - ρ²(b,bᵍᵗ)/c²

    Useful as a simpler alternative when aspect-ratio is less important or
    when dataset boxes exhibit high shape variance.
    """
    iou, union = box_iou(boxes1, boxes2)

    c1_x = (boxes1[:, 0] + boxes1[:, 2]) * 0.5
    c1_y = (boxes1[:, 1] + boxes1[:, 3]) * 0.5
    c2_x = (boxes2[:, 0] + boxes2[:, 2]) * 0.5
    c2_y = (boxes2[:, 1] + boxes2[:, 3]) * 0.5

    rho2 = ((c1_x[:, None] - c2_x[None, :]) ** 2 +
            (c1_y[:, None] - c2_y[None, :]) ** 2)

    lt = torch.min(boxes1[:, None, :2], boxes2[:, :2])
    rb = torch.max(boxes1[:, None, 2:], boxes2[:, 2:])
    wh = (rb - lt).clamp(min=0)
    c2_val = wh[:, :, 0] ** 2 + wh[:, :, 1] ** 2

    diou = iou - rho2 / (c2_val + eps)
    return diou
