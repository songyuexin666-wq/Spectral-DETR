...
def main():
    ...
    model = model_class(
        ...
        # IoU-aware classification (Fix 1)
        ia_bce_loss=model_config.get('ia_bce_loss', False),
        # LUE
        ...
        lue_giou_weighting=model_config.get('lue_giou_weighting', False),
        lue_giou_start_epoch=model_config.get('lue_giou_start_epoch', 40),
        lue_giou_warmup_epochs=model_config.get('lue_giou_warmup_epochs', 5),
        # DAFD
        use_dafd=model_config.get('use_dafd', False),
        ...
        dags_bonus_scale=model_config.get('dags_bonus_scale', 0.0),
        ...
        dqcd_decay_epochs=model_config.get('dqcd_decay_epochs', 25),
        dqcd_final_weight=model_config.get('dqcd_final_weight', -1.0),
        # SCU
        use_scu=model_config.get('use_scu', False),
        scu_salience_weight=model_config.get('scu_salience_weight', 0.1),
        scu_calib_slope=model_config.get('scu_calib_slope', -1.5),
        scu_calib_center=model_config.get('scu_calib_center', -4.2),
    )
    print("✓ 模型初始化完成")
    ...
    try:
        model.train(
            ...
            # 损失权重 + 匹配代价（从 configs/loss 读取，不存在的键不会覆盖模型内建默认值）
            **{k: loss_config[k] for k in [
                'cls_loss_coef', 'bbox_loss_coef', 'giou_loss_coef', 'lue_uncertainty_coef',
